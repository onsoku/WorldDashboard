...
function CustomContent({ x, y, width, height, name, category }: TreemapContentProps) {
...
function CustomTooltip({ active, payload, t }: { active?: boolean; payload?: TooltipPayloadItem[]; t: (k: string) => string }) {
...
export function KeywordMap({ keywords }: KeywordMapProps) {
      ...
      <div className="h-64">
        <ResponsiveContainer width="100%" height="100%">
          <Treemap data={data} dataKey="size" aspectRatio={4 / 3}
            content={<CustomContent x={0} y={0} width={0} height={0} name="" category="" />}>
